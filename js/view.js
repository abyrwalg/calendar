...
class View extends EventEmitter {
  ...
  inputDateHandler = (event) => {
    if (event.target.value) {
      ...
      this.selectedDay = document.querySelector(
        `td[data-date="${dateToString(this.today)}"]`
      );
      this.selectedDay.classList.add("active");
      this.showEventForm(
        this.selectedDay.getBoundingClientRect(),
        dateToString(this.today)
      );
    }
  };
  ...
  showSearchResults = (searchResults) => {
    ...
    searchResults.forEach((result) => {
      ...
      searchResultDateSpan.textContent = `${date.getDate()} ${
  ...
  createOverlay = (handler) => {
    ...
    overlay.style =
      "background-color: transparent; position: fixed; top: 0; left:0; right: 0; bottom: 0; display: block;";
    overlay.id = "overlay";
  ...
  calendarClickHandler = (event) => {
    ...
    const currentDate =
      event.target.tagName === "TD"
        ...
        : event.target.closest("td").dataset.date;
  ...
  showEventForm(position, currentDate) {
    ...
    const currentEvent = this.events.find(
      (eventItem) => eventItem.date === currentDate
    );
  ...
  addEventHandler = (event) => {
    ...
    const eventObject = {
      ...
      date: this.selectedDay
        ? this.selectedDay.dataset.date
        : dateToString(new Date(this.eventFormDate.value)),
    ...
    if (event.target.classList.contains("button")) {
      ...
      this.showCalendar();
      document
        .querySelector(`[data-date="${eventObject.date}"]`)
  ...
  createCalendarDay(text, date, events) {
    ...
    if (currentEvent) {
      ...
      if (currentEvent.participants.length > 50) {
        eventParticipants.textContent = `${currentEvent.participants.substring(
          0,
          50
        )}...`;
      } else {
  ...
  showCalendar(events = this.events) {
    ...
    let i = 0;
    const nextMonth = new Date(this.today.getTime());
    nextMonth.setMonth(this.today.getMonth() + 1);
    while (startDate.getMonth() !== nextMonth.getMonth()) {
      console.log(startDate.getMonth(), nextMonth.getMonth());
      const tr = document.createElement("tr");
      for (let j = 0; j < 7; j++) {
        const td = this.createCalendarDay(
          `${
            i === 0 ? `${days[j]}, ${startDate.getDate()}` : startDate.getDate()
          }`,
          startDate,
          events
        );
